...
class ApprovalRequest:
    ...
    def __init__(self, tool_name: str, action: str, parameters: Dict[str, Any], 
                 permission_level: str, reason: str = "", risk_level: RiskLevel = RiskLevel.MEDIUM):
...
class ApprovalFlow:
    ...
    def classify_risk(self, tool_name: str, action: str) -> RiskLevel:
        ...
        if tool_lower == "file_controller":
            ...
            if action_lower in {"delete", "delete_file", "move", "copy", "rename", "write", "create_file", "create_folder", "organize_desktop"}:
    ...
    def check_and_request_approval(self, tool_name: str, action: str, 
        ...
        is_allowed, message = check_action(
            self._current_permission_level, 
            action, 
            parameters
        )
        ...
        if is_allowed:
            ...
            if risk_level == RiskLevel.HIGH:
                return self._request_confirmation(
                    tool_name, action, parameters, risk_level,
        ...
        if "Confirmation required" in message or "blocked" in message.lower():
            ...
            return self._request_confirmation(
                tool_name, action, parameters, risk_level, message
            )
        
        return False, message
    
    def _request_confirmation(self, tool_name: str, action: str, 
                              parameters: Dict[str, Any], risk_level: RiskLevel,
        ...
        if decision is True:
            ...
            logger.warning(f"Approval request timed out: {tool_name}/{action} (risk: {risk_level.value})")
