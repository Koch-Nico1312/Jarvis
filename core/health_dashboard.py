...
class HealthStatusWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(f"background: {self.C.PANEL}; border: 1px solid {self.C.BORDER}; border-radius: 4px;")
    ...
    def _update_status(self):
        ...
        try:
            ...
            if report.get("api_key_present"):
                ...
                self._status_labels["api"].setStyleSheet(f"color: {self.C.GREEN}; background: transparent;")
            else:
                self._status_labels["api"].setText("✗ MISSING")
                self._status_labels["api"].setStyleSheet(f"color: {self.C.RED}; background: transparent;")
            
            # Audio status
            audio = report.get("audio", {})
            if audio.get("sounddevice_available"):
                self._status_labels["audio"].setText("✓ OK")
                self._status_labels["audio"].setStyleSheet(f"color: {self.C.GREEN}; background: transparent;")
            else:
                self._status_labels["audio"].setText("✗ N/A")
                self._status_labels["audio"].setStyleSheet(f"color: {self.C.TEXT_DIM}; background: transparent;")
            ...
            if memory.get("memory_file_exists"):
                ...
                self._status_labels["memory"].setStyleSheet(f"color: {self.C.GREEN}; background: transparent;")
            else:
                self._status_labels["memory"].setText("✗ EMPTY")
                self._status_labels["memory"].setStyleSheet(f"color: {self.C.RED}; background: transparent;")
            
            # RAG status
            features = report.get("features", {})
            if features.get("rag"):
                self._status_labels["rag"].setText("✓ ON")
                self._status_labels["rag"].setStyleSheet(f"color: {self.C.GREEN}; background: transparent;")
            else:
                self._status_labels["rag"].setText("○ OFF")
                self._status_labels["rag"].setStyleSheet(f"color: {self.C.TEXT_DIM}; background: transparent;")
                
        except Exception as e:
            # On error, show status as unknown
            for key in self._status_labels:
                self._status_labels[key].setText("?")
                self._status_labels[key].setStyleSheet(f"color: {self.C.TEXT_DIM}; background: transparent;")
