...
class ActionRecord:
    ...
    def __init__(self, action_type: ActionType, tool_name: str, action: str,
...
class ActionHistory:
    ...
    def record_action(self, tool_name: str, action: str, parameters: Dict[str, Any],
                     result: str = "", status: ActionStatus = ActionStatus.SUCCESS,
    ...
    def get_history(self, limit: int = 50, action_type: Optional[ActionType] = None) -> List[ActionRecord]:
        """
        Get action history.
    ...
    def retry_action(self, record_id: str, modified_parameters: Optional[Dict[str, Any]] = None) -> tuple[bool, str, Optional[ActionRecord]]:
        """
        ...
        with self._lock:
            ...
            if record.status != ActionStatus.FAILED:
                return False, f"Can only retry failed actions (current status: {record.status.value})", None
        ...
        logger.info(f"Action retry created: {record.tool_name}/{record.action} (original ID: {record.id}, new ID: {new_record.id})")
