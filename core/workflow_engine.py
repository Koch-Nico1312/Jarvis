...
from core.approval_flow import get_approval_flow, RiskLevel
...
from core.performance_tracker import get_performance_tracker, TaskStatus as PerfTaskStatus
from core.background_task_manager import get_background_task_manager, TaskPriority
...
class WorkflowEngine:
    ...
    def _execute_workflow(self, workflow: Workflow):
        ...
        try:
            while workflow.status == WorkflowStatus.RUNNING and not self._stop_event.is_set():
                ...
                if not ready_steps:
                    ...
                    if all(s.status in [StepStatus.COMPLETED, StepStatus.SKIPPED] for s in workflow.steps):
                        ...
                        workflow.completed_at = datetime.now()
                        workflow.duration_ms = (workflow.completed_at - workflow.started_at).total_seconds() * 1000
                        ...
                        if failed_steps:
                            workflow.status = WorkflowStatus.FAILED
                            workflow.error = f"Failed dependencies: {', '.join(s.name for s in failed_steps)}"
                ...
                # Update progress
                progress = workflow.get_progress_percent()
            ...
            self.action_history.record_action(
                ...
                parameters={"workflow_id": workflow.workflow_id, "name": workflow.name},
                result=f"Status: {workflow.status.value}",
                status=ActionStatus.SUCCESS if workflow.status == WorkflowStatus.COMPLETED else ActionStatus.FAILED
            )
    ...
    def cancel_workflow(self, workflow_id: str) -> bool:
        ...
        with self._lock:
            ...
            if workflow.status in [WorkflowStatus.PENDING, WorkflowStatus.QUEUED, WorkflowStatus.RUNNING, WorkflowStatus.PAUSED]:
    ...
    def retry_workflow(self, workflow_id: str) -> bool:
        ...
        with self._lock:
            ...
            if workflow.status == WorkflowStatus.FAILED and workflow.retry_count < workflow.max_retries:
                # Reset failed steps
    ...
    def get_active_workflows(self) -> List[Workflow]:
        ...
        with self._lock:
            return [w for w in self.workflows.values() if w.status in [WorkflowStatus.RUNNING, WorkflowStatus.PAUSED]]
