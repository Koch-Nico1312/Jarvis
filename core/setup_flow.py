...
class SetupReport:
    ...
    def _update_overall_status(self):
        ...
        required_failed = any(
            c.status == SetupStatus.FAILED and c.is_required 
            for c in self.checks
        )
        optional_failed = any(
            c.status == SetupStatus.FAILED and not c.is_required 
            for c in self.checks
        )
    ...
    def _check_audio_dependencies(self):
        ...
        try:
            ...
            if input_devices > 0 and output_devices > 0:
                ...
                check.message = f"Limited audio devices (input: {input_devices}, output: {output_devices})"
