...
from memory.hybrid_retrieval import get_hybrid_retrieval, RetrievalResult
...
class ContextModel:
    ...
    def add_signal(self, source: ContextSource, content: Any, confidence: float = 0.5, metadata: Dict[str, Any] = None):
...
class MultimodalContext:
    ...
    def decide_response_strategy(self, context: ContextModel) -> ContextDecision:
        ...
        if context.current_file and context.user_query:
            if any(word in context.user_query.lower() for word in ["analyze", "read", "summarize", "what"]):
                return ContextDecision.ANALYZE_FILE
        
        # Check if there's an image to analyze
        if context.current_image and context.user_query:
            if any(word in context.user_query.lower() for word in ["analyze", "what", "skin", "check"]):
    ...
    def generate_proactive_suggestions(self, context: ContextModel) -> List[str]:
        ...
        if context.memory_results:
            ...
            if top_result.relevance_score > 0.6:
                suggestions.append(f"I found relevant information about {top_result.entry.key}. Would you like me to elaborate?")
